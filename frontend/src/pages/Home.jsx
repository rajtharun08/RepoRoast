...
export default function Home({ onStartInterview, isLoading }) {
  ...
  return (
    <div className="min-h-[calc(100vh-57px)] bg-[#09090b] text-zinc-100 font-sans flex flex-col md:flex-row">
      
      {/* LEFT PANEL: Session Overview & Action */}
      <aside className="w-full md:w-80 lg:w-96 bg-zinc-900/80 border-b md:border-b-0 md:border-r border-zinc-800/80 p-6 flex flex-col justify-between shrink-0 space-y-6">
        <div className="space-y-6">
          ...
          <div className="bg-zinc-950 border border-zinc-800/80 rounded-xl p-4 space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs font-bold text-white font-mono bg-zinc-800 px-2 py-0.5 rounded-md">
                L{level}
              </span>
            </div>
        ...
        <div className="pt-4 border-t border-zinc-800/80">
          <button
            ...
            className="w-full bg-zinc-100 hover:bg-white text-zinc-900 font-bold text-sm py-3.5 rounded-xl shadow-sm flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-50 group"
          >
            {isLoading ? (
              <>
      ...
      <main className="flex-1 p-6 md:p-10 space-y-8 max-w-4xl overflow-y-auto">
        <div>
        ...
        <section className="bg-zinc-900/60 border border-zinc-800/80 rounded-2xl p-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            {/* Input Switcher */}
            <div className="flex items-center p-1 bg-zinc-950 border border-zinc-800 rounded-lg text-xs font-medium self-start sm:self-auto">
              <button
                type="button"
          ...
          {inputMode === 'search' ? (
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleFetchUserRepos(); }} className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-950 text-zinc-100 placeholder-zinc-500 text-xs pl-10 pr-4 py-2.5 rounded-xl border border-zinc-800 focus:outline-none focus:border-zinc-700 transition-all font-mono"
                  />
                </div>
                <button
                  type="submit"
                  disabled={isFetchingRepos}
                  className="bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold px-4 py-2.5 rounded-xl border border-zinc-700 transition-all cursor-pointer flex items-center gap-2 shrink-0 disabled:opacity-50"
                >
                  {isFetchingRepos ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Search className="w-3.5 h-3.5" />}
                  ...
                  {userRepos.map((repo) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-bold text-xs font-mono text-zinc-200 truncate">{repo.name}</span>
                        {selectedRepoUrl === repo.html_url && (
            ...
            <input
              ...
              className="w-full bg-zinc-950 text-zinc-100 placeholder-zinc-500 text-xs px-4 py-2.5 rounded-xl border border-zinc-800 focus:outline-none focus:border-zinc-700 font-mono transition-all"
            />
          )}
        </section>

        {/* Section 2: Escalation Level */}
        <section className="bg-zinc-900/60 border border-zinc-800/80 rounded-2xl p-6 space-y-5">
          <h3 className="text-sm font-bold text-white flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            >
              <div className="font-bold text-xs text-zinc-200">Levels 1–3</div>
              <div className="text-[11px] text-zinc-400 font-mono mt-0.5">Screening Phase</div>
            ...
            >
              <div className="font-bold text-xs text-zinc-200">Levels 4–7</div>
              <div className="text-[11px] text-zinc-400 font-mono mt-0.5">System Design</div>
              <p className="text-[11px] text-zinc-400 mt-1.5 leading-relaxed">
            ...
            >
              <div className="font-bold text-xs text-zinc-200">Levels 8–10</div>
              <div className="text-[11px] text-zinc-400 font-mono mt-0.5">Deep Code Review</div>
              <p className="text-[11px] text-zinc-400 mt-1.5 leading-relaxed">
          ...
          <div className="space-y-2 pt-1">
            ...
            <input
              ...
              className="w-full accent-zinc-100 bg-zinc-950 rounded-lg cursor-pointer h-2"
            />
          </div>
        </section>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="font-bold text-xs text-zinc-100">{title}</div>
                    {isSelected && <Check className="w-3.5 h-3.5 text-white shrink-0" />}
                  </div>
                  <span className="text-[10px] text-zinc-400 font-mono block">{badge}</span>
          ...
          {persona === 'Custom Persona' && (
            <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 space-y-2">
              ...
              <textarea
                ...
                className="w-full bg-zinc-900 text-zinc-100 placeholder-zinc-500 text-xs p-3 rounded-xl border border-zinc-800 focus:outline-none focus:border-zinc-700 font-mono leading-relaxed"
              />
            </div>
          )}
        </section>
